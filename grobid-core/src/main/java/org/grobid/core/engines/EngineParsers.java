...
public class EngineParsers implements Closeable {
    ...
    private PetitionerParser petitionerParser = null;
    private RespondentParser respondentParser = null;
    private PetitionerlawerParser petitionerlawerParser = null;
    private RespondentlawerParser respondentlawerParser = null;
    ...
    public PetitionerParser getPetitionerParser() {
        if (petitionerParser == null) {
            synchronized (this) {
                if (petitionerParser == null) {
                    petitionerParser = new PetitionerParser();
                }
            }
        }
        return petitionerParser;
    }

    public RespondentParser getRespondentParser() {
        if (respondentParser == null) {
            synchronized (this) {
                if (respondentParser == null) {
                    respondentParser = new RespondentParser();
                }
            }
        }
        return respondentParser;
    }

    public PetitionerlawerParser getPetitionerlawerParser() {
        if (petitionerlawerParser == null) {
            synchronized (this) {
                if (petitionerlawerParser == null) {
                    petitionerlawerParser = new PetitionerlawerParser();
                }
            }
        }
        return petitionerlawerParser;
    }

    public RespondentlawerParser getRespondentlawerParser() {
        if (respondentlawerParser == null) {
            synchronized (this) {
                if (respondentlawerParser == null) {
                    respondentlawerParser = new RespondentlawerParser();
                }
            }
        }
        return respondentlawerParser;
    }
    ...
    public void initAll() {
        ...
        petitionerParser = getPetitionerParser();
        respondentParser = getRespondentParser();
        petitionerlawerParser = getPetitionerlawerParser();
        respondentlawerParser = getRespondentlawerParser();
    ...
    public void close() throws IOException {
        ...
        if (petitionerParser != null) {
            petitionerParser.close();
            petitionerParser = null;
            LOGGER.debug("CLOSING petitionerParser");
        }
        if (respondentParser != null) {
            respondentParser.close();
            respondentParser = null;
            LOGGER.debug("CLOSING respondentParser");
        }
        if (petitionerlawerParser != null) {
            petitionerlawerParser.close();
            petitionerlawerParser = null;
            LOGGER.debug("CLOSING petitionerlawerParser");
        }
        if (respondentlawerParser != null) {
            respondentlawerParser.close();
            respondentlawerParser = null;
            LOGGER.debug("CLOSING respondentlawerParser");
        }
        if (affiliationAddressParser != null) {
