...
public class PetitionerParser {
	private static Logger LOGGER = LoggerFactory.getLogger(PetitionerParser.class);
    private final GenericTagger namesPetitionerParser;
    private final GenericTagger namesCitationParser;

    private static final Pattern ET_AL_REGEX_PATTERN = Pattern.compile("et\\.? al\\.?.*$");
	
    public PetitionerParser() {
        namesPetitionerParser = TaggerFactory.getTagger(GrobidModels.NAMES_PETITIONER);
    ...
    public List<Person> processing(List<LayoutToken> tokens, List<PDFAnnotation> pdfAnnotations, boolean head) {
        ...
        List<Person> fullPetitioners = null;
        try {
            ...
            if (StringUtils.isEmpty(sequence))
                return null;
            GenericTagger tagger = head ? namesPetitionerParser : namesCitationParser;
            String res = tagger.label(sequence);
//System.out.println(res);
            TaggingTokenClusteror clusteror = new TaggingTokenClusteror(head ? GrobidModels.NAMES_PETITIONER : GrobidModels.NAMES_CITATION, res, tokens);
            ...
            for (TaggingTokenCluster cluster : clusters) {
                ...
                if(pdfAnnotations != null) {
                    for (LayoutToken petitionersToken : cluster.concatTokens()) {
                        for (PDFAnnotation pdfAnnotation : pdfAnnotations) {
                            BoundingBox intersectBox = pdfAnnotation.getIntersectionBox(petitionersToken);
                            if (intersectBox != null) {
                                BoundingBox petitionersBox = BoundingBox.fromLayoutToken(petitionersToken);
                                if (intersectBox.equals(petitionersBox)) {
                                } else {
                                    double pixPerChar = petitionersToken.getWidth() / petitionersToken.getText().length();
                                    ...
                                    if (pdfAnnotation.getDestination() != null && pdfAnnotation.getDestination().length() > 0) {
                                        ...
                                        if (orcidMatcher.find()) {
                                            ...
                                            String newToken = petitionersToken.getText().substring(0, petitionersToken.getText().length() - charsCovered);        
                                            aut.setORCID(orcidMatcher.group(1) + "-"
                                                + orcidMatcher.group(2) + "-" + orcidMatcher.group(3)+ "-" + orcidMatcher.group(4));
                                            petitionersToken.setText(newToken);
                                        }
                                    }
                                }
                            }
                ...
                if (clusterContent.trim().length() == 0)
                    continue;
                if (clusterLabel.equals(TaggingLabels.NAMES_PETITIONER_MARKER)) {
                ...
                } else if (clusterLabel.equals(TaggingLabels.NAMES_PETITIONER_TITLE) || 
                ...
                } else if (clusterLabel.equals(TaggingLabels.NAMES_PETITIONER_FORENAME) || 
                    ...
                    aut.addLayoutTokens(cluster.concatTokens());
                } else if (clusterLabel.equals(TaggingLabels.NAMES_PETITIONER_MIDDLENAME) || 
                    ...
                    aut.addLayoutTokens(cluster.concatTokens());
                } else if (clusterLabel.equals(TaggingLabels.NAMES_PETITIONER_SURNAME) || 
                    ...
                    aut.addLayoutTokens(cluster.concatTokens());
                } else if (clusterLabel.equals(TaggingLabels.NAMES_PETITIONER_SUFFIX) || 
            ...
            if (fullPetitioners != null) {
                for(Person author : fullPetitioners) {
