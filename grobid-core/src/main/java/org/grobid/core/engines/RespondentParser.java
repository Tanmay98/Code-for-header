...
public class RespondentParser {
	private static Logger LOGGER = LoggerFactory.getLogger(RespondentParser.class);
    private final GenericTagger namesRespondentParser;
    private final GenericTagger namesCitationParser;

    private static final Pattern ET_AL_REGEX_PATTERN = Pattern.compile("et\\.? al\\.?.*$");
	
    public RespondentParser() {
        namesRespondentParser = TaggerFactory.getTagger(GrobidModels.NAMES_RESPONDENT);
    ...
    public List<Person> processing(List<LayoutToken> tokens, List<PDFAnnotation> pdfAnnotations, boolean head) {
        ...
        List<Person> fullRespondents = null;
        try {
            ...
            if (StringUtils.isEmpty(sequence))
                return null;
            GenericTagger tagger = head ? namesRespondentParser : namesCitationParser;
            String res = tagger.label(sequence);
//System.out.println(res);
            TaggingTokenClusteror clusteror = new TaggingTokenClusteror(head ? GrobidModels.NAMES_RESPONDENT : GrobidModels.NAMES_CITATION, res, tokens);
            ...
            for (TaggingTokenCluster cluster : clusters) {
                ...
                if(pdfAnnotations != null) {
                    for (LayoutToken respondentsToken : cluster.concatTokens()) {
                        for (PDFAnnotation pdfAnnotation : pdfAnnotations) {
                            BoundingBox intersectBox = pdfAnnotation.getIntersectionBox(respondentsToken);
                            if (intersectBox != null) {
                                BoundingBox respondentsBox = BoundingBox.fromLayoutToken(respondentsToken);
                                if (intersectBox.equals(respondentsBox)) {
                                } else {
                                    double pixPerChar = respondentsToken.getWidth() / respondentsToken.getText().length();
                                    ...
                                    if (pdfAnnotation.getDestination() != null && pdfAnnotation.getDestination().length() > 0) {
                                        ...
                                        if (orcidMatcher.find()) {
                                            ...
                                            String newToken = respondentsToken.getText().substring(0, respondentsToken.getText().length() - charsCovered);        
                                            aut.setORCID(orcidMatcher.group(1) + "-"
                                                + orcidMatcher.group(2) + "-" + orcidMatcher.group(3)+ "-" + orcidMatcher.group(4));
                                            respondentsToken.setText(newToken);
                                        }
                                    }
                                }
                            }
                ...
                if (clusterContent.trim().length() == 0)
                    continue;
                if (clusterLabel.equals(TaggingLabels.NAMES_RESPONDENT_MARKER)) {
                ...
                } else if (clusterLabel.equals(TaggingLabels.NAMES_RESPONDENT_TITLE) || 
                ...
                } else if (clusterLabel.equals(TaggingLabels.NAMES_RESPONDENT_FORENAME) || 
                    ...
                    aut.addLayoutTokens(cluster.concatTokens());
                } else if (clusterLabel.equals(TaggingLabels.NAMES_RESPONDENT_SURNAME) || 
                    ...
                    aut.addLayoutTokens(cluster.concatTokens());
                } else if (clusterLabel.equals(TaggingLabels.NAMES_RESPONDENT_SUFFIX) || 
            ...
            if (fullRespondents != null) {
                for(Person author : fullRespondents) {
